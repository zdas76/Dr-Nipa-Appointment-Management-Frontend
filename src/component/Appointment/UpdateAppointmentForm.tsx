...
export default function UpdateAppointmentForm({
  ...
  const { data: appData, isLoading: appIsLoading } =
  ...
  //     useGetLastVisitingDateQuery(data?.patientId as number, {
  //       skip: !data?.patientId,
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data.visitingDate) {
        try {
          formattedDate = new Date(data.visitingDate)
            .toISOString()
            .split("T")[0];
        } catch {
  ...
  }, [patientType, setValue, doctorinfo]);
  ...
  return (
    <Paper
      elevation={0}
      sx={{ p: 4, borderRadius: 4, border: "1px solid #e2e8f0" }}
    >
      <Typography
        variant="h5"
      ...
      {appIsLoading || !data ? (
        <Box sx={{ py: 4, textAlign: "center" }}>
          <Typography>Loading appointment data...</Typography>
        </Box>
      ) : (
