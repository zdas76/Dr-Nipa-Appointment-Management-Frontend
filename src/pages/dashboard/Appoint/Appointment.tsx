...
export default function AppointmentManagement() {
  ...
  const filteredPatients = (appointments?.data ?? []).filter(
    (row: TAppointment) => {
      ...
      <Box sx={{ mt: 3 }}>
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            <TableBody>
              {filteredPatients?.map((row: TAppointment, index: number) => (
                <TableRow
                  key={row.id}
                ...
                >
                  ...
                  <TableCell align="left">
                    {dayjs(row.visitingDate).format("DD-MM-YYYY")}
                  </TableCell>
                  <TableCell align="left">
                    {row.visitingTime || "N/A"}
                  </TableCell>
