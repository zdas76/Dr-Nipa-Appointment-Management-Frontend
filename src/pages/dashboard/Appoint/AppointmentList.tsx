...
const STATUS_CONFIG: Record<
  Status,
  { label: string; color: "warning" | "success" | "error" | "primary" }
> = {
...
export default function AppointmentList() {
  ...
  const handlePrint = (row: TAppointment) => {
    const params = new URLSearchParams({
      ...
      visitingDate:
        new Date(row.visitingDate).toISOString().split("T")[0] ?? "",
  ...
  const [menuAppointmentId, setMenuAppointmentId] = useState<number | null>(
    null,
  );
  ...
  const filteredAppointments = allData.filter((item) => {
    const searchTerm = searchText.toLowerCase();
    const patientName = (item.patientInfo?.name || "").toLowerCase();
    const patientContact = (
      item.patientInfo?.contactNumber || ""
    ).toLowerCase();
  ...
  const handlePrintAll = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Appointment List - ${date}`,
    pageStyle: `
      @page { size: A4; margin: 12mm; }
      body { font-family: Arial, sans-serif; }
      .print-section { display: block !important; }
      .print-table { width: 100%; border-collapse: collapse; }
      .print-table th, .print-table td { border: 1px solid #d1d5db; padding: 8pt 4pt; text-align: left; }
      .print-table th { background: #f8fafc; }
      .print-summary { margin: 0 0 18px; font-size: 14px; }
  ...
  const counts: Record<Status, number> = {
    BOOKED: filteredAppointments.filter(
      (i) => (i.status ?? "BOOKED") === "BOOKED",
    ).length,
  ...
  const printRows = ALL_STATUSES.flatMap((status) =>
    filteredAppointments.filter((item) => (item.status ?? "BOOKED") === status),
  );
  const printPages = Array.from(
    { length: Math.ceil(printRows.length / 20) },
    (_, pageIndex) => printRows.slice(pageIndex * 20, pageIndex * 20 + 20),
  );
  ...
  const selectedAppointment = allData.find(
    (item) => item.id === menuAppointmentId,
  );
  ...
  return (
    <Box>
      {/* Date Selector */}
      <Box className="flex items-center justify-evenly gap-4 py-2 my-2 w-full pr-15 no-print">
          ...
          return (
            <TabPanel key={s} value={s} sx={{ p: 0 }}>
              ...
              >
                <Table
                  sx={{ minWidth: 650 }}
                  aria-label="appointment list table"
                >
                  ...
                  <TableBody>
                    {filtered.length === 0 ? (
                      <TableRow>
                        ...
                        >
                          No {STATUS_CONFIG[s].label} appointments for this
                          date.
                        </TableCell>
                      </TableRow>
                    ) : (
                      filtered.map((row, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell>
                              {row.patientInfo?.patientId ?? "—"}
                            </TableCell>
                            <TableCell>
                              {row.patientInfo?.contactNumber ?? "—"}
                            </TableCell>
                            <TableCell>
                              {row.connectorInfo?.name
                                ? `${row.connectorInfo?.name} (${row.connectorInfo?.diagnosticName})`
                                : "—"}
                            </TableCell>
      ...
      <Box ref={printRef} sx={{ display: "none" }} className="print-section">
        <Typography variant="h5" sx={{ mb: 2, fontWeight: 700 }}>
          Appointment List
        </Typography>
        <Typography className="print-summary">
          Date: {date} | Search: {searchText || "All patients"}
        </Typography>
          ...
          <Box key={pageIndex} className="print-page">
            {ALL_STATUSES.map((status) => {
              const statusRows = rows.filter(
                (item) => (item.status ?? "BOOKED") === status,
              );
              ...
              return (
                <Box key={status} sx={{ mb: 3 }}>
                  ...
                  <Table className="print-table" sx={{ width: "100%" }}>
                    <TableHead>
                      <TableRow>
                    ...
                    <TableBody>
                      {statusRows.map((row, index) => (
                        <TableRow key={row.id}>
                          <TableCell>{index + 1}</TableCell>
                          <TableCell>{row.patientInfo?.name ?? "—"}</TableCell>
                          <TableCell>
                            {row.patientInfo?.patientId ?? "—"}
                          </TableCell>
                          <TableCell>
